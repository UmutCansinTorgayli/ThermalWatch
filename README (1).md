...
4. **(AMD Ryzen users)** If CPU temperature shows `0°C` or `None`, jump to [Settings → Download PawnIO Driver](#-the-amd-ryzen--pawnio-saga).
...
4. **(AMD Ryzen users)** If CPU temperature doesn't appear, jump to [Troubleshooting](#-the-amd-ryzen--pawnio-saga) and install the PawnIO driver from Settings.
